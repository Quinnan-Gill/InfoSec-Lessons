...
void decode_ethernet(const u_char *header_start) {
    ...
    for(i=1; i < ETHER_ADDR_LEN; i++)
        printf(":02%x", ethernet_header->ether_src_addr[i]);

    printf("\tDest: %02x", ethernet_header->ether_dest_addr[0]);
    for(i=1; i < ETHER_ADDR_LEN; i++)
        printf("\tDest: %02x", ethernet_header->ether_dest_addr[i]);
...
void decode_ip(const u_char *header_start) {
    ...
    id_header = (const struct ip_hdr *) header_start;
...
u_int decode_tcp(const u_char *header_start) {
    ...
    header_start = 4 * tcp_header->tcp_offset;

    printf("\t\t{{  Layer 4 :::: TCP Header  }}\n");
    printf("\t\t{ Src Port: %hu\t", ntohs(tcp_header->tcp_src_port));
    printf("Dest Port: %hu }\n", ntohs(tcp_header->tcp_dest_port));
    printf("\t\t{ Seq #: %u\t", ntohl(tcp_header->tcp_seq));
    printf("Ack #: %u\tFlags: ", header_size);
